...
The constructors have to provide data **for the new members and for the inherited members.
...
When a program constructs a derived-class object, it first constructs the base-class object. The base-class object should be constructed **before** the program enters the body of the derived-class ocnstructor. C++ uses the member initializer list syntax:
...
The example uses the implicit destructor. **Destroying an object occurs in the opposite order used to construct an object. That is, **the body of the derived-class is executed first, and then the base-class destructor is called automatically.

> ##### Note

> When creating a derived-class object, a program first **calls the base-class constructor** and then **calls the derived-class constructor**. **A derived-class constructro ALWAYS calls a base-class constructor**. You can use the **initializer list syntax to indicate _which_ base-class constructor to use.** Otherwise, the default base-class construcotr is used.

> When an object of a derived class expires, the program **frist calls the derived-class destructor and then calls the base-class destructor.
...
> A class can pass values back only to its **immediate base class**. If you don't supply a base-class constructor in a member initializer list, the program uses the default base-class constructor. **The member initializer list syntax can be used only in constructors**.
...
A base-class pointer or reference can point to or refer to a derived-class object without explicit type cast.
...
_is-implemented-as-a relationship: a stack and an array. For example, array indexing is not a stack property. A proper approach would by to hide the array implementation by giving the stack a private `Array` object member.

_uses-a_ relationship: a printer and computer. You might devise friend functions or classes to handle communication.

## Polymorphic Public Inheritance

You can have multiple behaviors for a method, depending on the context. This is termed _polymorphic_.

There are two independent key mechanisms for implementing polymorphic public inheritance:
...
**The two prototype** indicates that there will be two separate method definitions. The qualified names will be `Brass::ViewAcct()` and `BrassPlus::ViewAcct()`. A program will use the object to determine which version to use:
...
void BrassPlus::ViewAcct() const
{
	...
	cout << "Owed ot bank: $" << owesBank << endl;
	cout.precision(3);
...
Keep in mind that the **derived class does not have direct access to private base-class data**; the derived class has to use **base-class public methods to access the data**.

The means of access depends on the method.
...
Suppose you would like to manage a mixture of `Brass` and `BrassPlus` accounts. You can't have a single array that holds a mixture of `Brass` and `BrassPlus` objects because each item in an array has to be the same type. **However, you can create an array of pointers-to-`Brass`.
...
The code in Listing 13.10 that **uses `delete` to free the objects allocated by `new` illustrates why the base class should have a virtual destructor, even if no destructor appears to be needed.

If the destructors are not virtual, then **just the corresponding to the pointer type is called.** If the destructors are virtual, the destructor corresponding to the object type is called.
...
Upcasting also takes place for function calls with base-class references or pointers as parameters.
...
First, let's consider efficient. For a program to be able to make a runtime decision, it has to have some way to keeptrack of what sort of object a base-class pointer or reference refers to, and that entails some extra processing overhead. You should therefor go to virtual functions onl if the program design needs them.

Next, let's consider the conceptual model. When you design a class, you may have member functions that you don't want redefined in derived classes. By making these function nonvirtual, you accomplish two things. First, you make it more effiecient. Second, **you announce that it is your intention that this function not be refined.**
...
Friends can't be virtual functions because only members can be virtual functions.
...
Or perhaps you won't get a warning. Either wy, the code has the following implications:
...
The difference between `private` and `protected` comes into play only within the classes derived from the base class. **Members of a derived class can access protected members of a base class directly, but they cannot directly access private members of the base class.** So members in the protected category behave like privae members as far as the outside world is concerned but behave like public members as far as derived classes are concerned.
...
What the two classes have in common are the coordinates of the center of the shape; a `Move()` method, and an `Area()` method. However, **the `Area()` method works differently for the two classes**. Indeed, the `Area()` method can't even be implemented for the ABC because it doesn't have the necessary data member. C++ has a way to provide an unimplemented function by using a `pure virtual function`. A pure virtual function has `= 0` at the end of its declaration:
...
You see a complete example of an ABC. **All methods and data members that are common to both the `Brass` and the `BrassPlus` classes.** `AcctABC` also provides a protected member function to handle the formatting previously handled by nonmember functions.
...
The ABC methodology is a much more systematic, disciplined way to approach inheritance. One school of thought holds that if you design an inheritance hierarchy of classes, **the only concrete classes should be those that never serve as a base class.**
...
If the derived class does not itself use dynamic memory allocation, **you don't need to take special steps.**
...
In this case, you do have to define an explicit destructor, copy constructor, and assignement operator for the derived class.
...
What this actually does is, assign a `hasDMA` object to another `hasDMA` object **using the assignment operator defined in base class.** This workds because the derived class inherits the assignement operator. But to specify it's the base-class assignment to use, you **have use the function notation and scope-resolution operator.** Otherwise, the compiler will use the default assignment for the derived-class, **resulting in a recursive function.**
...
If you don't define **any** constructors, the ocmpiler defines a default constructor. You need a default constructor to use the following:
...
A class copy constructor is used when new object is to be created (**make a duplicate of an object**). That is, in the following situations:
...
All situation requires a copy of an object.

If a program use a copy constructor, the program defines a copy construcotr that perform memberwise initialization. **If a member is itself a class object, then memberwise initilization uses the copy constructor defined for that particular class. If some pointer members are initialized with `new`, you need to define you own copy constructor to perform deep copying.
...
Default assignment uses memberwise assignment. If a member is itself a class object, then default memberwise assignment uses the assignment operator defined for that particular class.

**Assignment operator is in itself a function**:
...
**Conversion is a process of creating new object like making a copy**. So like copy constructor, it's called when:

* Initialize an object to another type
* Pass anther type to an object parameter by value
* Return another type when the return type is a class
* Assign an object to a value of another type (Actually is passing function arguments).
...
**Unlike initializations, assignment happends only when `=` symbol is explicitly used.**
...
**VERY IMPORTSNT: derived-class methods can use the scopy-resolution operator to invoke public and protected base-class methods, even if this method is redefined in the derived class.** That's why this statement runs:
...
The protected members act like public members as far as a derived class is concerned, but they act like private member s for the world at large.
...
6. If the base class and a derived-class both define a method with the same name, the newly defined version will be invoked by a derived-class object. **BUt the derived-class method can access base-class methods by using the scope-resolution operator**.
