...
There is an alternative to wrapping one function call in another: use a `using` declaration to announce that a particular base-class member can be used by the derived class, even though the derivation is private:
...
**This would make noth bersions available.	
